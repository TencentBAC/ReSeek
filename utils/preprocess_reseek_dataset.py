...
DEFAULT_USER_CONTENT_PREFIX = """Answer the given question step by step.
    Instructions:
    ...
    b. If the information is useful BUT you still need more details, you MUST search again with <search>...</search>.
    c. If the information is not useful, you MUST search again with <search>...</search>. You MUST NOT provide an answer in this case.
    5. Provide your final answer in <answer>...</answer> tags. The <answer> tag marks the end of the task. After providing the <answer>, you MUST stop and generate no further text.
...
def process_single_row(row, current_split_name, row_index):
    """
    Process a single row of data for ReSeek-like format.
    ...
    # Process data source
    data_source_tagged = "ReSeek_" + str(row.get("data_source", ""))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Download ReSeek from HuggingFace, process, and save to Parquet.")
    parser.add_argument(
        "--hf_repo_id", default="TencentBAC/ReSeek_train_test", help="HuggingFace dataset repository ID."
    )
    parser.add_argument(
        "--local_dir",
        default="./data/ReSeek_processed_direct_v1",
