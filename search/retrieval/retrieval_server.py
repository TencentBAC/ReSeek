...
class Encoder:
    ...
    def encode(self, query_list: List[str], is_query=True) -> np.ndarray:
        ...
        # Process input based on different models
        if "e5" in self.model_name.lower():
            ...
            query_list = [f"{query}" for query in query_list]

        embeddings = self.model.encode(
            query_list, batch_size=32, show_progress_bar=True, convert_to_numpy=True, normalize_embeddings=True
        )
        # Ensure output is float16 type, consistent with index
