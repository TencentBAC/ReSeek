...
def load_model(
    ...
    if use_vllm:
        # 使用 vLLM 加载模型 (需要 vllm>=0.8.5)
        llm = LLM(
            model=model_path,
            task="embed",  # 指定为 embedding 任务
            tensor_parallel_size=tensor_parallel_size,
            dtype="float16" if use_fp16 else "auto",
...
class Index_Builder:
    ...
    def _prepare_vllm_components(self):
        """预热 vLLM 模型，进行一次小批量推理以初始化所有组件"""
        try:
            print("正在预热 vLLM embedding 模型...")
            # 使用官方 embed API 进行预热
            dummy_input = ["这是一个测试文本用于预热模型"]
            _ = self.encoder.embed(dummy_input)
            print("vLLM 模型预热完成")
            
        except Exception as e:
            print(f"vLLM 模型预热失败: {e}")
            print("继续执行，可能首次推理会稍慢")

    def encode_all_vllm(self):
        """使用 vLLM 进行批量编码 (需要 vllm>=0.8.5)
        
        使用 vLLM 的官方 embed API，简单高效
        """
        all_embeddings = []
        
        print(f"开始使用 vLLM embed API 编码文档...")
        
        try:
            # 分批处理
            for start_idx in tqdm(range(0, len(self.corpus), self.batch_size), desc='vLLM Embedding:'):
                ...
                # 使用 vLLM 的 embed 方法直接获取 embeddings
                outputs = self.encoder.embed(batch_data)
            ...
            print(f"vLLM embed 失败: {e}")
            print("请确保使用 vllm>=0.8.5 并且模型支持 embedding 任务")
            print("回退到 transformers 实现...")
            return self.encode_all_transformers()
        
        all_embeddings = np.concatenate(all_embeddings, axis=0)
        return all_embeddings

    def encode_all_transformers(self):
        """原有的 transformers 实现"""
        if self.gpu_num > 1:
            print("Use multi gpu!")
        ...
        for start_idx in tqdm(range(0, len(self.corpus), self.batch_size), desc='Transformers Inference:'):
    ...
    def build_dense_index(self):
        ...
        # 如果使用 vLLM，预先准备模型组件以提升推理速度
        if self.use_vllm:
            self._prepare_vllm_components()
        if self.embedding_path is not None:
            if self.use_vllm:
                # 对于 vLLM，从 model_config 获取 hidden_size
...
def main():
    ...
    # vLLM parameters
    parser.add_argument('--use_vllm', default=False, action='store_true', help='使用 vLLM 进行推理以提升速度')
    parser.add_argument('--tensor_parallel_size', type=int, default=1, help='vLLM 张量并行大小')
    parser.add_argument('--max_model_len', type=int, default=2048, help='vLLM 最大模型长度')
